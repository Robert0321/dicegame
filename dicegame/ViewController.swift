...
//  dicegame
//
//  Created by robert on 2022/11/2.
//
...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var betSmallSegmentedControl: UISegmentedControl!
    ...
    override func viewDidLoad() {
        ...
        for d in 0...2 {
            ...
            //讓骰子呈45度角
            diceImageView[d].transform = CGAffineTransform.identity.rotated(by: CGFloat.pi/180 * 45)
            //骰子隨機顏色
            diceImageView[d].tintColor = UIColor(hue: CGFloat(Double.random(in: 0...1)), saturation: CGFloat(Double.random(in: 0...1)), brightness: CGFloat(Double.random(in: 0...1)), alpha: 1)
        }
        ...
        controller.addAction(UIAlertAction(title: "好啊 !", style: .default, handler: { _ in
            ...
            getChipsController.addAction(UIAlertAction(title: "呼叫彼得潘", style: .default, handler: { _ in
                ...
                if getChipsController.textFields?[0].text == "peterpan" {
                    //填寫完成後 再出現一個確認完成填寫視窗
                    let doneController = UIAlertController(title: "Hooray !", message: "獲得15000籌碼 !", preferredStyle: .alert)
    ...
    fileprivate func play() {
           if chips > 0 {
               ...
               UIViewPropertyAnimator.runningPropertyAnimator(withDuration: 1.2, delay: 0, animations:{
                   ...
                   for d in 0...2 {
                       for _ in 1...20 {
                           ...
                           self.diceImageView[d].transform = CGAffineTransform.identity.translatedBy(x: CGFloat.random(in: -15...15), y: CGFloat.random(in: -100...100)).scaledBy(x: scale, y: scale).rotated(by: CGFloat.pi/180 * CGFloat.random(in: -360...360))
                           //讓影像隨機變色
                           self.diceImageView[d].tintColor = UIColor(hue: CGFloat(Double.random(in: 0...1)), saturation: CGFloat(Double.random(in: 0...1)), brightness: CGFloat(Double.random(in: 0...1)), alpha: 1)
                       }
                   }
               }, completion: nil)
               //設定點數隨機變換
               for dice in diceImageView {
               ...
               case 0:
                   switch betSmallSegmentedControl.selectedSegmentIndex {
                       //賭小100
                   case 0:
                       switch totalPoint {
                       case 3...9:
                           ...
                           chipsLabel.text = "\(chips) 💸"
                           //設定提醒視窗樣式及內容
                           let controller = UIAlertController(title: "YOU WIN !", message: "\(totalPoint)點,加1000籌碼", preferredStyle: .alert)
                           //新增動作按鈕
                       ...
                       default:
                           ...
                           chipsLabel.text = "\(chips) 💸"
                           if chips > 0 {
                               let controller = UIAlertController(title: "YOU LOSE !", message: "\(totalPoint)點,減1000籌碼", preferredStyle: .alert)
                               controller.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
                               controller.addAction(UIAlertAction(title: "同樣賭金再玩一次", style: .default, handler: { _ in
                                   self.play()
                               }))
                   ...
                   default:
                       ...
                       case 3...9:
                           chips += 3000
                           chipsLabel.text = "\(chips) 💸"
                           let controller = UIAlertController(title: "YOU WIN !", message: "\(totalPoint)點,加3000籌碼", preferredStyle: .alert)
                           controller.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
                           controller.addAction(UIAlertAction(title: "同樣賭金再玩一次", style: .default, handler: { _ in
                               self.play()
                           }))
               ...
               default:
                   ...
                   default:
                       ...
                       default:
                           ...
                           chipsLabel.text = "\(chips) 💸"
                           if chips > 0 {
                               let controller = UIAlertController(title: "YOU LOSE !", message: "\(totalPoint)點,減3000籌碼", preferredStyle: .alert)
                               controller.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
                               controller.addAction(UIAlertAction(title: "同樣賭金再玩一次", style: .default, handler: { _ in
                                   self.play()
                               }))
